...
const SatelliteMesh = () => {
  ...
  useEffect(() => {
    ...
    for (let r = 0; r < rows; r++) {
      for (let c = 0; c < cols; c++) {
        ...
        nodes.push({
          ...
          cy: y,
          size: hexSize,
          phase: (c * 0.4 + r * 0.6) + Math.random() * 0.5,
          neighbors: [],
        });
      }
    }
    ...
    const drawHex = (
      x: number, y: number, size: number, alpha: number, wave: number, time: number
    ) => {
      ...
      // Small circles on each edge midpoint
      for (let i = 0; i < 6; i++) {
        const next = (i + 1) % 6;
    ...
    const draw = () => {
      ...
      for (let waveLayer = 0; waveLayer < 3; waveLayer++) {
        ...
        const alpha = 0.025 - waveLayer * 0.007;
      ...
      for (let wl = 0; wl < 2; wl++) {
        ...
        for (let x = 0; x <= cw; x += 2) {
          const y = baseY + Math.sin(x * 0.015 + t * 1.2 + wl) * 8
            + Math.sin(x * 0.008 + t * 0.7) * 5;
      ...
      // Draw connections between hex nodes
      for (let i = 0; i < nodes.length; i++) {
        const n = nodes[i];
      ...
      // Draw hex nodes with ocean wave motion
  ...
  return (
    <div className="absolute top-0 left-1/2 -translate-x-1/2 z-10 pointer-events-none"
      style={{ width: "min(600px, 80vw)", height: "220px" }}
    >
      {/* Fade-out mask */}
